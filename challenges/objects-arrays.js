...
const lowerCase = [];
zooAnimals.map((ele) => {
  return lowerCase.push((ele.animal_name).toLowerCase());
})
...
zooAnimals.filter((ele) => {
  if (ele.population < 5) {
    return largerPopulation.push(ele.animal_name);
  }
})
...
*/
const populationTotal = zooAnimals.reduce((acc, ele) => {
